...
class Attention(nn.Module):
    ...
    def forward(self, emb):
        ...
        # maybe sigmmoid的
        attn_probs = nn.functional.sigmoid(attn_score)
...
def create_embedding_dict(config, linear=False, action=None):
    # action 用来指定单任务，如果是但任务，则 linear 的 logit 维度应该为 1
    direct_emb = 4 if not action else 1
...
class Linear(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.embedding_dict = create_embedding_dict(config, linear=False, action=config.action)
        self.weight = nn.Parameter(torch.Tensor(4, 4 if not config.action else 1))
        self.trans_weight = nn.Parameter(torch.Tensor(80, 4 if not config.action else 1))
...
class Expert(nn.Module):
    ...
    def __init__(self, config: Namespace):
        ...
        self.attent_layer = nn.Sequential(nn.Linear(256+128, 32),
                                          nn.BatchNorm1d(32),
                                          nn.ReLU(),
                                          nn.Linear(32, 32),
                                          nn.BatchNorm1d(32),
...
class MultiDeepFM(nn.Module):
    ...
    def __init__(self, config: Namespace):
        ...
        self.feed_fc = nn.Sequential(
            ...
            nn.Linear(256, 128),
            nn.GELU(),
            nn.Linear(128, 4 if not config.action else 1)
        )
    ...
    def forward(
        ...
        for layer in self.expert_list:
            self.expert_feat.append(layer(svd_features, deepwalk_features, dense_features, sparse_features,
                feed_embedding, ocr, ocr_char, asr, asr_char, desc, desc_char))
        self.expert_feat = torch.stack(self.expert_feat, 1)
        task_logits = []
        for i in range(self.task_num):
            attent = self.gates_inear[i](gate_inputs)
            attent = F.softmax(attent).unsqueeze(-1)
            task_input = (attent * self.expert_feat).sum(1).squeeze(1)
            task_logits.append(self.tower_list[i](task_input))    

        if self.config.action:
            labels = labels[self.config.action]
        else:
        ...
        return pos_loss, neg_loss, loss, nums_loss, logits, labels, nums_logits
